...
function buildBricksForLevel() {
  ...
  // Aspect Ratio calculations to prevent squishing / stretching (Object-Fit Cover style)
  const targetAreaW = cols * brickW;
  const targetAreaH = rows * brickH;
  const targetAspectRatio = targetAreaW / targetAreaH; // e.g. 800 / 336 = 2.38
  const imgAspectRatio = imgW / imgH;

  let cropW, cropH, cropX, cropY;

  if (imgAspectRatio > targetAspectRatio) {
    // Image is wider than target area (landscape)
    cropH = imgH;
    cropW = imgH * targetAspectRatio;
    cropX = (imgW - cropW) / 2;
    cropY = 0;
  } else {
    // Image is taller than target area (portrait)
    cropW = imgW;
    cropH = imgW / targetAspectRatio;
    cropX = 0;
    // We center the crop vertically, but bias it slightly upwards (0.35)
    // to capture faces/focus areas which are usually in the top half of portrait photos.
    cropY = (imgH - cropH) * 0.35;
    if (cropY < 0) cropY = 0;
  }

  const sourceSliceW = cropW / cols;
  const sourceSliceH = cropH / rows;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // Crop coordinates in the source image, offset by crop X/Y
      const sx = cropX + (c * sourceSliceW);
      const sy = cropY + (r * sourceSliceH);
